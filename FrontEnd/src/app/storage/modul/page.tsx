// ========================ini code app/storage/modul/page.tsx (Full Interactive Fix)======
"use client";
...
export default function StaticModuleContent() {
  ...
  const [activeModul, setActiveModul] = useState(allModules[0]);
  ...
  const stepsData = [
      ...
      content: (
        <div className="space-y-8!">
          <section>
            ...
            <p className="text-lg! md:text-xl! font-bold! leading-relaxed! text-slate-600! dark:text-slate-400! mb-8!">Memahami fundamental dari {activeModul.toLowerCase()} dalam rekayasa struktur modern.</p>
            <div className="text-slate-700! dark:text-slate-300! font-bold! leading-relaxed!">
              Modul ini akan membahas secara mendalam tentang prinsip-prinsip {activeModul} sebagai landasan utama dalam analisis statika struktur untuk mencapai keseimbangan sempurna.
            </div>
          </section>
      ...
      content: (
        <div className="space-y-8!">
          <section>
            ...
            <p className="text-slate-700! dark:text-slate-300! font-bold! leading-relaxed!">Setiap struktur bangunan harus memenuhi kaidah statika yang ketat. Pada bagian ini, kita akan membedah bagaimana elemen bekerja dalam mendistribusikan {activeModul.toLowerCase()}.</p>
          </section>
    ...
    { id: 5, title: "Rangkuman", content: <div className="space-y-8!"><section><h2 className="text-2xl! md:text-3xl! font-outfit! font-black! uppercase! tracking-tight! text-slate-900! dark:text-white! mb-6! flex! items-center! gap-4!"><span className="w-2! h-8! bg-violet-600! rounded-full!" /> Kesimpulan Materi</h2><p className="text-slate-700! dark:text-slate-300! font-bold!">Pastikan pemahaman Anda tentang {activeModul.toLowerCase()} sudah matang sebelum melanjutkan ke simulasi battlefield.</p></section><Card className="p-8! bg-slate-900! text-white! border-white/10! shadow-2xl!"><h4 className="font-outfit! text-xl! font-black! uppercase! mb-4! tracking-tight!">Engineer's Note</h4><p className="text-slate-400! font-black! italic!">"Ketelitian dalam analisis {activeModul.toLowerCase()} adalah fondasi utama keselamatan bangunan."</p></Card></div> }
  ];
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.4 }}>
        <div className="flex! flex-col! md:flex-row! justify-between! items-start! md:items-center! gap-6! mb-10!">
          ...
          <nav className="flex! items-center! gap-2! text-[10px]! md:text-xs! font-black! uppercase! tracking-widest! text-slate-400!">
            ...
            <ChevronRight className="w-3! h-3!" />
            <span className="text-violet-500! border-b! border-violet-500/30! uppercase!">{activeModul}</span>
          </nav>
        ...
        <div className="mb-12!">
          ...
          <h1 className="font-outfit! text-4xl! md:text-7xl! font-black! tracking-tight! uppercase! text-slate-900! dark:text-white! mb-6!">{activeModul}</h1>
          <div className="flex! gap-3!">
          ...
          <aside className="w-full! lg:w-80! space-y-8!">
            <Card className="p-8! border-slate-200! dark:border-white/5! bg-white! dark:bg-slate-950! premium-shadow!">
              ...
              <div className="space-y-6!">
                {allModules.map((moduleName, index) => (
                  <button 
                    key={index} 
                    onClick={() => {
                        setActiveModul(moduleName);
                        setStep(1); // 2. Reset step to 1
                        window.scrollTo({ top: 0, behavior: 'smooth' });
                    }}
                    className={`w-full flex! items-center! gap-4! group! text-left! transition-all! ${activeModul === moduleName ? "opacity-100 scale-105" : "opacity-40 hover:opacity-100"}`}
                  >
                    <div className={`w-12! h-12! rounded-xl! flex! items-center! justify-center! border! border-slate-200! transition-all! ${activeModul === moduleName ? "bg-violet-600! border-violet-600! shadow-lg! shadow-violet-500/30" : "bg-slate-50! dark:bg-slate-900! group-hover:bg-violet-600! dark:border-white/10!"}`}>
                      <BookOpen className={`w-5! h-5! transition-colors! ${activeModul === moduleName ? "text-white!" : "text-slate-400! group-hover:text-white!"}`} />
                    </div>
                    <div className="overflow-hidden!">
                      <p className={`font-black! text-sm! uppercase! font-outfit! truncate! transition-colors! ${activeModul === moduleName ? "text-violet-600!" : "text-slate-900! dark:text-white! group-hover:text-violet-600!"}`}>
                        {index + 1}. {moduleName}
                      </p>
                    </div>
                  </button>
            ...
            <Link href="/storage/materi">
              <button className="w-full! py-4! rounded-2xl! border-2! border-slate-200! dark:border-white/10! font-black! uppercase! text-xs! tracking-widest! hover:bg-slate-50! dark:hover:bg-white/5! text-slate-500! transition-all!">
                <ChevronLeft className="w-4! h-4! inline! mr-2!" /> Katalog Projek
              </button>
            </Link>
          </aside>
        </div>
...
// ==========================ini code app/storage/modul/page.tsx selesai ==============
