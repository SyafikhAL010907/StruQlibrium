...
function UjiLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  // Extract question number from pathname (e.g., /UjiKompetensi/soal/1 -> 1)
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] text-slate-900 dark:text-white pb-20">
      ...
      <div className="container mx-auto px-6 mt-8">
        <div className="flex items-center justify-between mb-2">
           ...
           <span className="text-[10px] font-black text-violet-600 uppercase tracking-widest">{currentSoal}/40 SELESAI</span>
        </div>
        <div className="w-full h-2 bg-slate-200 dark:bg-white/10 rounded-full overflow-hidden">
           <motion.div 
             initial={{ width: 0 }}
             animate={{ width: `${(currentSoal / 40) * 100}%` }}
             className="h-full bg-linear-to-r from-violet-600 to-fuchsia-500"
           />
        </div>

        {/* Question Numbers Navigation List */}
        <div className="mt-6 flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {Array.from({ length: 40 }, (_, i) => i + 1).map((num) => {
            const isActive = currentSoal === num;
            return (
              <Link
                key={num}
                href={`/UjiKompetensi/soal/${num}`}
                className={`flex-none w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center text-sm sm:text-base font-black snap-center transition-all duration-300 ${
                  isActive 
                    ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/30 scale-110 z-10' 
                    : 'bg-white dark:bg-slate-900/50 text-slate-400 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 border border-slate-200 dark:border-white/5'
                }`}
              >
                {num}
              </Link>
