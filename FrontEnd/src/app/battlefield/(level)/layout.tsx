...
export default function BattlefieldLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <BattlefieldContext.Provider value={contextValue}>
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-6 md:py-12 w-full overflow-visible">
        {/* HUD System */}
        <div className="flex flex-col sm:flex-row justify-between items-center sm:items-center gap-4 sm:gap-0 mb-6 sm:mb-8 px-4 sm:px-6 bg-white/80 dark:bg-slate-950/90 border border-slate-200 dark:border-white/10 p-3 sm:p-4 rounded-2xl shadow-sm dark:shadow-none backdrop-blur-md">
          <div className="flex gap-4 sm:gap-8 items-center">
            ...
            <div className="flex items-center gap-2 font-black font-mono text-lg text-rose-600 dark:text-rose-400">
              <Timer className="w-5 h-5" /> {difficulty === 'hard' ? "05:00" : difficulty === 'medium' ? "10:00" : "15:00"}
            </div>
          </div>
          <div className="font-outfit font-black tracking-[0.4em] text-[10px] md:text-xs uppercase text-slate-500 dark:text-slate-400">
            OPERASI: {difficulty.toUpperCase()}
          </div>
        </div>

        {/* Progress Board */}
        <div className="mb-8 lg:mb-12 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 p-4 sm:p-6 lg:p-8 rounded-3xl sm:rounded-[3rem] shadow-2xl shadow-slate-900/10 backdrop-blur-md">
              ...
              {[...Array(TOTAL_STEPS)].map((_, i) => {
                ...
                return (
                  <Link key={s} href={`/battlefield/${difficulty}/${s}`}
                    className={`w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full sm:rounded-3xl flex items-center justify-center font-black text-sm sm:text-base z-10 transition-all duration-500 border-2 ${s === currentStep
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 lg:gap-8 items-stretch w-full">
          {/* Aside (Sidebar) */}
          <aside className="lg:col-span-4 flex flex-col gap-6 sm:gap-8 h-full order-1 lg:order-1 w-full">
            <div className="bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-white/5 p-6 sm:p-8 lg:p-12 rounded-3xl sm:rounded-[3rem] shadow-xl backdrop-blur-2xl relative overflow-visible flex flex-col h-full min-h-[400px]">
              ...
              <div className="mt-auto pt-8 space-y-4 relative z-10 w-full">
                ...
                <div className="pt-2 grid grid-cols-2 gap-2 sm:gap-3 scale-95 sm:scale-100 origin-bottom">
          ...
          {/* Main (Visualizer) */}
          <main className="lg:col-span-8 h-full flex flex-col order-2 lg:order-2 w-full">
            <div className="bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/5 p-6 sm:p-10 lg:p-12 rounded-3xl sm:rounded-[4rem] shadow-xl backdrop-blur-3xl flex flex-col h-full min-h-[400px] lg:min-h-[600px] relative ring-1 ring-slate-100 dark:ring-white/10 overflow-visible">
